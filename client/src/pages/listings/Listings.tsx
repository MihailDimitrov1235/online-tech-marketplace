      ...
      <div className="flex gap-8 w-full items-start px-14 pt-8 pb-12">
        <div className="w-80 shrink-0 sticky top-20 rounded-2xl bg-white dark:bg-zinc-900 border border-border shadow-sm p-5 flex flex-col gap-6">
          <div className="flex items-center justify-between">
            ...
            {hasFilters && (
              <button
                onClick={() => { setConditions([]); setMinPrice(""); setMaxPrice("") }}
          ...
          <div>
            <p className="text-xs font-medium text-muted uppercase tracking-wide mb-3">Condition</p>
            <div className="flex flex-col gap-2">
              {CONDITIONS.map(c => (
                <label key={c} className="flex items-center gap-2.5 cursor-pointer group">
                  <div
                    onClick={() => toggleCondition(c)}
                    className={`w-4 h-4 rounded-sm border flex items-center justify-center shrink-0 transition-colors ${conditions.includes(c)
          ...
          <div>
            <p className="text-xs font-medium text-muted uppercase tracking-wide mb-3">Price range</p>
